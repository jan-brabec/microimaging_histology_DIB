...
* We have prepared a function that allows us to view H&E, VEGF as well as Mean diffusivity and in-plane fractional anisotropy (FAIP). It allows also one to zoom-in also on particular regions of the histology of the DTI map by clicking on one of the maps. Navigate yourself to the folder **Step_5_View_data**
...
* The data folder also contains **DTI_raw**.
  ...
  * **processed** sub-folder contains processed raw DTI data by the DTI pipeline in [Step_1_Process_DTI](https://github.com/jan-brabec/microimaging_histology_DIB/tree/main/Step_1_Process_DTI).
...
* These are stored in the *MR* structure in *MR.mat* and contain
  ...
  * All 13 diffusion-weighted measurements before averaging or calculating maps in the field *DWI*.
...
In this step, the histology image is cropped and rotated to approximately match the MR image without its shearing or image deformation. This is performed by running *Register* which performs the following steps:
...
* It coregisters image B (MRI) to image A (H&E) by creating a deformable mesh grid over image B (MRI) based on defined landmarks. It also crops image A (H&E).
...
* The GUI can be controlled by pressing:
  ...
  * p - goes to the previous landmark
  * i - shows the interpolation grid
  * r - computes the final result and stores it in the folder **coreg_fine**
...
QuPath is an open software for Bioimage analysis that was used to generate cell density maps from the related research article. However, it can also be used to view the histology slides - both raw or coregistered ones. Here are some instructions if you want to open any of the histology slides:
